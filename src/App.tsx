...
function SceneApp() {
  ...
  useEffect(()=>{if(hudVisible||!tbRef.current)return
...
export default function App() {
  const [page, setPage] = useState<'scene' | 'admin'>('scene')
  ...
  if (page === 'admin') {
    return <AdminLayout user={user} loading={loading} menu={menu} canManage={canManage} login={login} logout={logout} />
  }
