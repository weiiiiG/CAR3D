...
**Base URL:** `http://localhost:5180/api`（与前端同端口，Next.js API Routes）
...
## Users（需 JWT 认证，super_admin/admin 角色）

| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/api/users` | 用户列表 |
| POST | `/api/users` | 新增用户（含密码 bcrypt 加密） |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/api/dashboard` | 获取所有仪表盘图表配置（合并为 key→data 对象） |
...
| GET | `/api/views/:key` | 单个视角详情（含关联覆盖） |
| POST | `/api/views` | 新增视角 |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/seed` | 重新导入所有初始数据（views + mock_vehicles + users + dashboard_config） |
